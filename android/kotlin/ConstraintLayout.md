...
- 기본적으로 각 위젯, parent간에 간격은 `spread`로 균등하게 나누어지게 된다.
- 이는 각 위젯간에 체인이 걸려있다고 말한다.
- 현재는 textView1, 2가 수직으로 체인이 걸려있고 이는 `app:layout_constraintVertical_chainStyle="spread"` attribute가 있는 것과 같다.
